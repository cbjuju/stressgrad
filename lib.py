...
def get_elements_and_nodes(odb):
    ...
    # The entries of the following three lists contain more information than
    # necessary
    ...
    # The entries of the following three lists contain only information
    # relevant to the repair
    allNodes    = []
    allElements = []

    """
    Each entry of allNodes is a list with two elements in it : first the node
    label and second a list of the node coordinates.
    Each entry of allElements is also a list of two members : the first is the
    element number and the second is the element nodes. distortedElements is
    built the same way as allElements.
    """
...
def get_stress_data(odb, number_of_elements):
    ...
    for num in range(number_of_elements):
        ...
        entry_ip1 = [id_ip1] + list(stress_at_ip1)
        entry_ip2 = [id_ip2] + list(stress_at_ip2)
        entry_ip3 = [id_ip3] + list(stress_at_ip3)
        entry_ip4 = [id_ip4] + list(stress_at_ip4)
...
def get_displacement_data(odb, number_of_nodes):
    ...
    for num in range(number_of_nodes):
        ...
        entry = [node_number] + list(displacements)
